  ...
  const handleCreateOrUpdateCoupon = async () => {
    try {
      if (selectedCoupon) {
        ...
        const response = await fetch(`https://localhost:7001/api/Coupons/${selectedCoupon.couponId}`, {
          method: 'PUT',
          headers: {
        ...
        if (response.ok) {
          setCoupons((prevCoupons) =>
            prevCoupons.map((coupon) =>
              coupon.couponId === selectedCoupon.couponId ? { ...coupon, ...newCoupon } : coupon
            )
          );
        ...
        <div className="bg-gray-700 p-6 rounded mt-60 w-2/3 ml-96">
          ...
          <label className="mb-2 text-white">
            ...
            <input
              ...
              onChange={(e) => setNewCoupon({ ...newCoupon, discountAmount: parseInt(e.target.value, 10) })}
          ...
          <label className="mb-2 text-white">
            ...
            <input
              ...
              onChange={(e) => setNewCoupon({ ...newCoupon, minAmount: parseInt(e.target.value, 10)})}
              className="border p-2 ml-9  text-black"
            />
          </label>
          <button onClick={handleCreateOrUpdateCoupon} className="bg-green-500 text-white px-4 py-2 rounded mt-4 w-1/2">
            Submit
          </button>
          <button onClick={() => setModalIsOpen(false)} className="mt-2 text-gray-600 px-4 py-2  bg-red-500 w-1/2">
            Cancel
          </button>
          </div>
        </div>
